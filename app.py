...
# 3. חיבור דינמי ל-3 הגיליונות המעובדים שלך מגוגל שיטס
...
def load_all_processed_sheets():
    ...
    df_f = pd.read_csv(URL_DATASET)
    df_w = pd.read_csv(URL_WEEKLY)
    return df_f, df_w


try:
    df_fires, df_weekly = load_all_processed_sheets()
except Exception:
    # פתרון גיבוי אמין למניעת קריסות
    df_fires = pd.DataFrame({'frp': [65.0, 150.0, 20.0], 'confidence': [85, 95, 60]})
    df_weekly = pd.DataFrame({'Area': [120, 240, 180]})


# 4. מנוע הלוגיקה וה-ML של סוכן ה-FireMate (עם תיקון ה-KeyError של העמודות!)
class FireMateIntelligenceEngine:
    def __init__(self, df_fires, df_weekly):
        ...
        # תיקון חכם ל-KeyError: איתור אוטומטי של שמות העמודות גם אם הן באותיות גדולות או בשם ארוך
        self.frp_col = self._find_column_match(['frp', 'FRP', 'fire_radiative_power_mw', 'fire_radiative_power'])
        self.conf_col = self._find_column_match(['confidence', 'Confidence', 'confidence_pct', 'Confidence_pct'])

    def _find_column_match(self, options):
        for opt in options:
            if opt in self.df_fires.columns:
                return opt
        # אם לא מצא, ייקח את העמודה המספרית הראשונה כברירת מחדל כדי לא לקרוס
        num_cols = self.df_fires.select_dtypes(include=[np.number]).columns
        return num_cols[0] if len(num_cols) > 0 else self.df_fires.columns[0]
    ...
    def compute_similarity(self):
        """מנוע דמיון פריטים (Cosine Similarity) המותאם לעמודות הדינמיות שלך"""
        # שליפת הפיצ'רים על בסיס העמודות שזוהו חכם
    ...
    def run_anomaly_detection(self):
        """איתור אנומליות וחריגות בנתונים השבועיים (Z-Score)"""
    ...
    def generate_tactical_response(self, text):
        ...
        # חילוץ הערך של ה-FRP ההיסטורי בצורה בטוחה
        hist_frp_val = twin_case[self.frp_col] if self.frp_col in twin_case else "N/A"

        res = f"### 🤖 ניתוח סוכן חכם - FireMate AI\n\n"
        res += f"🧬 **מנוע דמיון פריטים (ML):** האירוע הושווה מול {len(self.df_fires):,} רשומות NASA מהדאטה שלכן. נמצאה שריפה תאומה עם **{sim_score * 100:.1f}% אחוז דמיון קוסינוס** (FRP היסטורי: {hist_frp_val}).\n"
...
# 5. כותרות ומבנה העמוד העליון ב-HTML
